<?php
  ...
  if (empty($name) && empty($email) && empty($password) && empty($confirm_password) && empty($cpf) && empty($data_nasc) && empty($cep) && empty($street) && empty($city) && empty($state) && empty($cel) && empty($code)) {
    ...
    echo json_encode(array("status" => "error", "message" => true, "text" => "Preencha o código de verificação"));
    session_destroy();
  }
  else if ($code != @$_SESSION['code']['code']) {
    echo json_encode(array("status" => "error", "message" => true, "text" => "Código de verificação inválido"));
  }
  else if (@$_SESSION['code']['user_number'] != $cel) {
    echo json_encode(array("status" => "error", "message" => true, "text" => "Celular inválido"));
  }
  else {
